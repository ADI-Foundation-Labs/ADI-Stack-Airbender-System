...
pub fn run_big_int_test_impl(
    ...
    let binary = std::fs::read("../examples/big_int/app.bin").unwrap();
    ...
    let mut delegation_circuits = vec![];
    {
        ...
        if !for_gpu_comparison {
            serialize_to_file(&circuit, "bigint_delegation_circuit_layout.json");
        }

        let delegation_type = BIGINT_OPS_WITH_CONTROL_CSR_REGISTER;
        let description = DelegationProcessorDescription {
            delegation_type: BIGINT_OPS_WITH_CONTROL_CSR_REGISTER,
        ...
        delegation_circuits_eval_fns.insert(
            delegation_type,
            super::bigint_with_control_delegation_with_gpu_tracer::witness_eval_fn,
        );
    }
    dbg!("Running witness evaluation");
    ...
    for work_type in delegation_circuits.into_iter() {
        ...
        for witness in work_type.work_units.into_iter() {
            ...
            if !for_gpu_comparison {
                serialize_to_file(&proof, "bigint_delegator_proof");
            }
...
#[test]
fn run_basic_bigint_delegation_test() {
    run_big_int_test_impl(None, None);
}
