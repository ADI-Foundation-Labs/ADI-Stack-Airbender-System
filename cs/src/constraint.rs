//! Human riendly polynomial algebra for expressing constraints.
//!
//! Term: the atomic piece of a polynomial either a constant or a single monomial 
//! coeff * x1 * x2 * ... . 
//! Terms follow the usual polynomial laws: multiplication is associative (and
//! commutative over the variable multiset) and distributes over addition. We
//! keep terms normalized.
...
impl<F: PrimeField> Term<F> {
    ...
    /// Why do we need to sort inner[..degree]?
    /// Because multiplication is commutative, x*y and y*x must be represented identically. Sorting inner[..degree] makes the representation unique.
