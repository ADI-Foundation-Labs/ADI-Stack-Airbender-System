use super::cpu_worker::{
    CyclesChunk, InitsAndTeardownsChunk, NonDeterminism,
};
...
use super::precomputations::{
    get_common_precomputations, get_delegation_circuit_precomputations,
...
struct BinaryHolder {
    ...
    precomputations: HashMap<CircuitType, CircuitPrecomputations>,
}
...
pub struct ExecutionProver<K: Debug + Eq + Hash> {
    ...
    binary_holders: RwLock<HashMap<K, BinaryHolder>>,
...
impl<K: Clone + Debug + Eq + Hash> ExecutionProver<K> {
    ...
    pub fn with_configuration(configuration: ExecutionProverConfiguration) -> Self {
        ...
        let binary_holders = RwLock::new(HashMap::new());
        ...
        let free_allocator_sender_ref = &free_allocator_sender;
        (0..host_allocators_count).into_par_iter().for_each(|_| {
            ...
            free_allocator_sender_ref.send(allocator).unwrap();
        });
...
#[cfg(test)]
mod tests {
    use crate::execution::prover::ExecutionProver;
    use crate::tests::init_logger;

    #[test]
    fn test_execution_prover() {
        init_logger();
        let _prover = ExecutionProver::<usize>::new();
    }
}