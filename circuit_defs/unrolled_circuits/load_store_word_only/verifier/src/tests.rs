...
fn test_query_values_offsets_pointer_arithmetic() {
    ...
    let mut current_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, query_index) / core::mem::size_of::<u32>()) };
    ...
    for (i, &offset_increment) in BASE_CIRCUIT_QUERY_VALUES_OFFSETS.iter().enumerate() {
        ...
        match i {
            0 => {
                ...
                let expected_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, setup_leaf) / core::mem::size_of::<u32>()) };
            ...
            idx if idx == LEAF_SIZE_SETUP => {
                ...
                let expected_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, witness_leaf) / core::mem::size_of::<u32>()) };
            ...
            idx if idx == LEAF_SIZE_SETUP + LEAF_SIZE_WITNESS_TREE => {
                ...
                let expected_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, memory_leaf) / core::mem::size_of::<u32>()) };
            ...
            idx if idx == LEAF_SIZE_SETUP + LEAF_SIZE_WITNESS_TREE + LEAF_SIZE_MEMORY_TREE => {
                ...
                let expected_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, stage_2_leaf) / core::mem::size_of::<u32>()) };
                assert_eq!(current_ptr, expected_ptr, "stage_2_leaf pointer mismatch");
            }
            idx if idx == LEAF_SIZE_SETUP + LEAF_SIZE_WITNESS_TREE + LEAF_SIZE_MEMORY_TREE + LEAF_SIZE_STAGE_2 => {
                // After stage_2_leaf elements, we should be at quotient_leaf
                let expected_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, quotient_leaf) / core::mem::size_of::<u32>()) };
                assert_eq!(current_ptr, expected_ptr, "quotient_leaf pointer mismatch");
            }
            idx if idx == LEAF_SIZE_SETUP + LEAF_SIZE_WITNESS_TREE + LEAF_SIZE_MEMORY_TREE + LEAF_SIZE_STAGE_2 + LEAF_SIZE_QUOTIENT => {
                // After quotient_leaf elements, we should be at fri_oracles_leafs
                let expected_ptr = unsafe { base_ptr.add(offset_of!(QueryValuesInstance, fri_oracles_leafs) / core::mem::size_of::<u32>()) };
                assert_eq!(current_ptr, expected_ptr, "fri_oracles_leafs pointer mismatch");
            }
            _ => {}
        }
    }
